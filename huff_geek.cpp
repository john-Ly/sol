...
// huffman 的类型可以联系前面  morse + strjoin
// 两个类型的总和
// 找出第 K 个字符的编码 可以使用动规法
// 出现频次较高的字符采用更短的编码, 反之采用较长的编码 (可变长编码)
...
struct MinHeapNode {
    ...
    MinHeapNode(char data, unsigned freq,
                MinHeapNode* lhs = nullptr, MinHeapNode* rhs = nullptr)
                : data(data), freq(freq), left(lhs), right(rhs) {}
