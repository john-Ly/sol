#ifndef CA_UWATERLOO_ALUMNI_DWHARDER_N_PUZZLE
#define CA_UWATERLOO_ALUMNI_DWHARDER_N_PUZZLE
...
namespace Data_structures {
	...
	class N_puzzle {
		...
		public:
			...
			template <int M>
			friend std::ostream &operator<<( std::ostream &, N_puzzle<M> const & );
	};
	...
	template <int N>
	std::ostream &operator<<( std::ostream &out, N_puzzle<N> const &pz ) {
		if ( !pz.valid() ) {
			for ( int i = 0; i < N; ++i ) {
				for ( int j = 0; j < N; ++j ) {
					out << "X\t";
