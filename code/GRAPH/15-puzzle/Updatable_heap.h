#ifndef CA_UWATERLOO_ALUMNI_DWHARDER_UPDATABLE_HEAP
#define CA_UWATERLOO_ALUMNI_DWHARDER_UPDATABLE_HEAP
...
namespace Data_structures {
	...
	template <typename T, int M>
	Updatable_heap<T, M>::Updatable_heap( T const &pz ):
	heap_size( 0 ),
	maximum_heap_size( 0 ) {
		for ( int i = 0; i < M; ++i ) {
			hash_table[i] = 0;
	...
	void Updatable_heap<T, M>::push( T const &pz, int path_length ) {
		...
		if ( ptr == 0 ) {
			...
			maximum_heap_size = std::max( maximum_heap_size, size() );
		} else {
			if ( !ptr->visited ) {
	...
	int Updatable_heap<T, M>::length( T const &pz ) const {
		...
		return ( ptr == 0 ) ? INT_MAX : ptr->length();
	}

	template <typename T, int M>
	typename Updatable_heap<T, M>::Step *Updatable_heap<T, M>::pointer( T const &pz ) const {
		for ( Step *ptr = ptr = hash_table[pz.hash() & (M - 1)]; ptr != 0; ptr = ptr->next ) {
			if ( ptr->element == pz ) {
				return ptr;
			}
		}
